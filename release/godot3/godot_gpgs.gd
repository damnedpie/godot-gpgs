...
func initialize() -> void:
	...
	_gpgs.connect("achievementsLoaded", self, "_onAchievementsloaded")
	_gpgs.connect("achievementsRevealed", self, "_onAchievementsRevealed")
	_gpgs.connect("achievementsUnlocked", self, "_onAchievementsUnlocked")
	_gpgs.connect("eventsLoaded", self, "_onEventsLoaded")
	_gpgs.connect("eventsLoadedByIds", self, "_onEventsLoadedByIds")
	_gpgs.connect("leaderboardsScoreSubmitted", self, "_onLeaderboardsScoreSubmitted")
	_gpgs.connect("leaderboardsScoreLoaded", self, "_onLeaderboardsScoreLoaded")
	_gpgs.connect("leaderboardsAllLoaded", self, "_onLeaderboardsAllLoaded")
	_gpgs.connect("leaderboardsLoaded", self, "_onLeaderboardsLoaded")
	_gpgs.connect("leaderboardsPlayerCenteredScoresLoaded", self, "_onLeaderboardsLoadPlayerCenteredScores")
	_gpgs.connect("leaderboardsTopScoresLoaded", self, "_onLeaderboardsLoadTopScores")
	_gpgs.connect("playersCurrentLoaded", self, "_onPlayersCurrentLoaded")
	_gpgs.connect("playersFriendsLoaded", self, "_onPlayersFriendsLoaded")
	_gpgs.connect("playersSearched", self, "_onPlayersSearched")
	_gpgs.connect("signInUserAuthenticated", self, "_onSignInUserAuthenticated")
	_gpgs.connect("signInRequestedServerSideAccess", self, "_onSignInRequestedServerSideAccess")
	_gpgs.connect("snapshotsGameSaved", self, "_onSnapshotsGameSaved")
	_gpgs.connect("snapshotsGameLoaded", self, "_onSnapshotsGameLoaded")
	_gpgs.connect("snapshotsConflictEmitted", self, "_onSnapshotsConflictEmitted")
	_gpgs.connect("imageStored", self, "_onImageStored")
	_gpgs.initialize()
...
func playersCompareProfileWithAlternativeNameHints(other_player_id: String, other_player_in_game_name: String, current_player_in_game_name: String) -> void:
	_gpgs.playersCompareProfileWithAlternativeNameHints(other_player_id, other_player_in_game_name, current_player_in_game_name)
...
func snapshotsSaveGame(file_name: String, description: String, save_data: PoolByteArray, played_time_millis: int,	progress_value: int) -> void:
...
func _onPlayersSearched(player: String) -> void:
	emit_signal("players_searched", JSON.parse(player).result)
